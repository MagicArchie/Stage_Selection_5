...
function preload() {
  ...
  for (let i = 0; i < stageCount; i++) {
    const imagePath = `materials/images/Stg_${i + 1}E.png`;
    try {
...
function setup() {
  ...
  for (let i = 0; i < stageCount; i++) {
    ...
    stages.push({
      ...
      interactive: i < 5 // Set interactivity to true for the first five nodes, false for others
    });
  }
...
function mouseClicked() {
  ...
  if (clickedNode !== -1) {
    console.log(`Clicked Inactive Node ${clickedNode + 1}: ${stages[clickedNode].link}`);
    ...
    if (clickedNode !== -1) {
      console.log(`Clicked Node ${clickedNode + 1}: ${stages[clickedNode].link}`);
      ...
      setTimeout(function () {
        window.location.href = stages[clickedNode].link;
      }, 1100);
    } else if (
...
// Example: To turn off interactivity for all nodes except the first five
